...
#'
#' @export
plot_split_support <- function(main_tree, n_trees, support,
  lab_size = 2, supp_size = 2, xlim_max = 0.5, hjust = 1) {
  edge <- data.frame(main_tree$edge, support = rep(NA, nrow(main_tree$edge)))
  ind <- match(as.integer(names(branch_support)), edge$X2)
  support_prop <- round(support$branch_support/n_trees, 2)
